...
const addCart = asyncHandler(async (req, res) => {
  ...
  const checkQualityOfSize = product.quantity.find(
    (el) => el.size === parseInt(size)
  );
  // check input size have exist in product
  if (!checkSize.includes(parseInt(size))) {
    return res.status(400).json({ msg: `chỉ có thể ở  ${checkSize}` });
  }
  ...
  if (checkQualityOfSize && count > checkQualityOfSize.numberOfSize) {
    return res.status(400).json({
      msg: `Maximum quantity: ${checkQualityOfSize.numberOfSize}`,
      value: count,
    });
  }
  //passed check
  ...
  const alreadyProduct = user.Cart.find(
    (el) => el.product.toString() === pid && el.size == size
  );
  if (alreadyProduct) {
